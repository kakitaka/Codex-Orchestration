...
class NativeRoutingTests(unittest.TestCase):
    ...
    def test_preset_setup_status_and_disable_preserve_concurrency(self) -> None:
        ...
        state = json.loads(state_path.read_text(encoding="utf-8"))
        self.assertEqual(state["schema"], 8)
        self.assertEqual(state["policy_version"], 8)
    ...
    def test_schema_six_preset_remains_disableable_without_luna_defaults(self) -> None:
        ...
        legacy_state.pop("token_profile")
...
if "features" in sys.argv and "list" in sys.argv:""",
    ...
    def test_legacy_state_schemas_upgrade_to_current_without_losing_restore(self) -> None:
        for legacy_schema in (1, 2, 3):
            with self.subTest(schema=legacy_schema):
                ...
                self.assertEqual(upgraded["schema"], 8)
                self.assertEqual(upgraded["policy_version"], 8)
                self.assertIsNone(upgraded["token_profile"])
    ...
    def test_explicit_token_profile_uses_combined_schema_and_is_preserved(self) -> None:
        setup = self.run_script(
            "--executor-model",
            "gpt-5.6-luna",
            "--token-profile",
        ...
        self.assertEqual(updated["schema"], 8)
