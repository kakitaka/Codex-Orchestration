...
class SkillContractTests(unittest.TestCase):
    ...
    def test_terra_luna_sol_escalation_preset_is_strict_and_nonroutine(self) -> None:
        ...
        self.assertIn("gpt-5.6-luna` at `max` Executor", SKILL)
        self.assertIn("gpt-5.6-sol` at `max` Advisor", SKILL)
        self.assertIn("ordinary work has no Advisor approval loop", SKILL)
        self.assertIn("security, auth, or secrets", SKILL)
        self.assertIn("database schema or\ndestructive migration", SKILL)
        self.assertIn("public API or backward-compatibility risk", SKILL)
        self.assertIn("cross-subsystem architecture change", SKILL)
        self.assertIn("repeated implementation or test failures", SKILL)
        ...
        self.assertIn("do not substitute a\nmodel", SKILL)
        self.assertIn("no worker or concurrency limit", SKILL)
        self.assertIn("features.multi_agent = true", SKILL)
        self.assertIn("agents.enabled = true", SKILL)
        self.assertIn('agents.default_subagent_model = "gpt-5.6-luna"', SKILL)
        self.assertIn("agents.default_subagent_reasoning_effort = \"max\"", SKILL)
        self.assertIn("omit direct `model` and `reasoning_effort`", SKILL)
        self.assertIn("Profile boundary crossings are never in-place", SKILL)
        self.assertIn("features.multi_agent = true", README)
        self.assertIn("schema 7 must carry policy version 7", REFERENCE)
        self.assertIn("reversible callable-Luna controls", REFERENCE)
