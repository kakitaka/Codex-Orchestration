...
| A profile state is forged, incomplete, or altered. | Schema 8 is the sole writer. It accepts the exact preset name, Luna @ Max Executor, null Planner/Advisor/Designer, and exact paired snapshots for the multi-agent/default-subagent controls; malformed state blocks repair and disable writes. Schemas 1–7 remain read-only compatibility shapes. | Schema 6 preset state remains readable without callable Luna defaults; schema 7/8 preset state requires them. |
| Luna defaults are saved while the multi-agent feature is disabled or the client cannot parse them. | A fresh preset reversibly owns only mode/usage/metadata/namespace and `features.multi_agent`, `agents.enabled`, and exact Luna @ Max defaults. It never owns MCP launchers or `expose_spawn_agent_model_overrides`; every known client is probed before writes, and aliases or fallback are rejected. | Configuration readback is not proof of a later live spawn. |
...
| A claimed Terra root is mistaken for persisted or live proof. | Policy labels Terra @ Medium as the normal expected next-task selection only and keeps an explicit selected task model or effort unchanged. | Runtime confirmation requires a separately observed live task. |
