...
Capability-test the complete four-field preset with every known shared-config client under an isolated `CODEX_HOME`. `--allow-incompatible-client` requires explicit user acceptance. Disable must remain available even when compatibility probes fail.
